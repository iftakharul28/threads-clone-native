  ...
  return (
    <Pressable
      ...
      <View style={{ flexShrink: 1, gap: 6 }}>
        <PostHeading name={thread.author.name} verified={thread.author.verified} createdAt={thread.createdAt} />
        <Text>{thread.content}</Text>
        {thread.image && <Image source={thread.image} style={{ width: '100%', minHeight: 300, borderRadius: 10 }} placeholder={blurhash} contentFit='cover' transition={500} />}
        <BottomIcons />
...
export function PostHeading({ name, createdAt, verified }: { name: string; createdAt: string; verified: boolean }) {
  return (
    <View
      style={{
        flexDirection: 'row',
...
export function PostFooter({ replies, likes }: { replies: number; likes: number }) {
  return (
